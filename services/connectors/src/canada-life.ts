...
import {
  canadaLifePayloadSchema,
  type AgentObservation,
  type CanadaLifePayload
} from "@tenio/contracts";

import { decryptCredential } from "./crypto.js";
import { getConnectorCredentialRow } from "./db.js";

const CONNECTOR_ID = "canada-life-groupnet-browser";
const CONNECTOR_NAME = "Canada Life GroupNet";

// TODO: Confirm production URL with Canada Life integration guide
const PORTAL_BASE_URL =
  process.env.CANADA_LIFE_PORTAL_URL ?? "https://groupnet.canada-life.ca";
...
export const canadaLifeExecuteRequestSchema = z.object({
...
export type CanadaLifeExecuteRequest = z.infer<typeof canadaLifeExecuteRequestSchema>;
...
async function runPortalAutomation(
  ...
  try {
    ...
    // ── 1. Navigate to login ──────────────────────────────────────────────────
    // TODO: Verify login URL path against Canada Life GroupNet portal
    ...
    // ── 2. Fill login form ────────────────────────────────────────────────────
    // TODO: Verify username input selector against Canada Life GroupNet portal
    await page.waitForSelector(
      "#username, input[name='username'], input[type='email'], #userId",
      { timeout: stepTimeoutMs }
    );
    await page.fill(
      "#username, input[name='username'], input[type='email'], #userId",
    ...
    // TODO: Verify password input selector against Canada Life GroupNet portal
    await page.fill(
      "#password, input[name='password'], input[type='password']",
      password
    );

    // TODO: Verify submit button selector against Canada Life GroupNet portal
    await page.click(
      "button[type='submit'], input[type='submit'], .btn-login, #signInButton"
    );
    ...
    await Promise.race([
      page
        .waitForSelector(
          ".error-message, .alert-danger, [data-testid='login-error'], .form-error",
          { timeout: 4_000 }
        )
        .then(() => "error"),
      page
        .waitForURL(/dashboard|home|groupnet|benefits|provider/, {
          timeout: stepTimeoutMs
        })
        .then(() => "success")
    ]).catch(() => undefined);

    // TODO: Verify login error selector against Canada Life GroupNet portal
    ...
    // TODO: Verify claims navigation selector against Canada Life GroupNet portal
    await page.waitForSelector(
      "[data-nav='claims'], a[href*='claim'], a[href*='benefit'], nav a",
      { timeout: stepTimeoutMs }
    );
    await page.click(
      "a[href*='claim-status'], a[href*='claims'], [data-nav='claims']"
    );

    // ── 5. Search by claim number ─────────────────────────────────────────────
    // TODO: Verify claim number input selector against Canada Life GroupNet portal
    await page.waitForSelector(
    ...
    // TODO: Verify search submit button selector against Canada Life GroupNet portal
    await page.click(
      ".claim-search button[type='submit'], .search-claims-btn, button.search, #searchClaims"
    );

    // ── 6. Wait for results ───────────────────────────────────────────────────
    // TODO: Verify results container selector against Canada Life GroupNet portal
    ...
    if (!resultsEl) {
      // TODO: Verify not-found message selector against Canada Life GroupNet portal
      const notFoundEl = await page.$(
        ".no-results, .claim-not-found, [data-testid='no-results'], .empty-state"
      );
      if (notFoundEl) {
        throw new PortalError(
          "data_missing",
          `Claim ${claimNumber} not found in Canada Life GroupNet portal`
        );
      }
      throw new PortalError(
        "portal_changed",
        "Could not find results container — Canada Life GroupNet portal layout may have changed"
      );
    }

    // ── 7. Extract claim status ───────────────────────────────────────────────
    // TODO: Verify status cell selector against Canada Life GroupNet portal
    ...
    if (!statusEl) {
      throw new PortalError(
        "portal_changed",
        "Could not find claim status element — Canada Life GroupNet portal layout may have changed"
      );
    }
    ...
    if (!statusText?.trim()) {
      throw new PortalError(
        "portal_changed",
        "Claim status element was empty — Canada Life GroupNet portal layout may have changed"
      );
    }

    // ── 8. Extract paid amount ────────────────────────────────────────────────
    // TODO: Verify paid amount selector against Canada Life GroupNet portal
    ...
    // TODO: Verify denial reason selector against Canada Life GroupNet portal
...
function buildSummary(payload: CanadaLifePayload): string {
  const base = `Canada Life GroupNet: claim ${payload.claimNumber ?? "unknown"}`;
...
export async function executeCanadaLife(
  request: CanadaLifeExecuteRequest
  ...
  if (!row) {
    return buildFailureObservation(
      "authentication",
      false,
      `No Canada Life GroupNet credentials configured for org ${request.orgId}`,
      Date.now() - startMs
    );
  }
  ...
  try {
    ...
    return buildFailureObservation(
      "authentication",
      false,
      "Failed to decrypt Canada Life credentials",
      Date.now() - startMs
    );
  }
  ...
  try {
    ...
    const parsed = canadaLifePayloadSchema.safeParse({
      connectorId: CONNECTOR_ID,
    ...
    const payload: CanadaLifePayload = parsed.success
      ? parsed.data
      : {
          connectorId: CONNECTOR_ID,
          claimNumber: request.claimContext.claimNumber,
    ...
    const summary = isTimeout
      ? "Canada Life GroupNet portal timed out"
      : `Canada Life automation failed: ${err instanceof Error ? err.message : "unknown error"}`;
