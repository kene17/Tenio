 ...
 * ciphertext in the returned Buffer so it can be stored as a single BYTEA
 * column and recovered during decryption.
 */
export async function encryptCredential(
  ...
  if (key.length !== sodium.crypto_secretbox_KEYBYTES) {
    throw new Error(
      `Credential encryption key must be ${sodium.crypto_secretbox_KEYBYTES * 2} hex characters (${sodium.crypto_secretbox_KEYBYTES} bytes)`
    );
  }
...
/**
 * Decrypts a Buffer produced by encryptCredential. Throws on authentication
 * failure (wrong key or tampered ciphertext).
 */
