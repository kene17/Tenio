...
import {
  manulifePayloadSchema,
  type AgentObservation,
  type ManulifePayload
} from "@tenio/contracts";

import { decryptCredential } from "./crypto.js";
import { getConnectorCredentialRow } from "./db.js";

const CONNECTOR_ID = "manulife-groupbenefits-browser";
const CONNECTOR_NAME = "Manulife Group Benefits";

// TODO: Confirm production URL with Manulife integration guide
const PORTAL_BASE_URL =
  process.env.MANULIFE_PORTAL_URL ?? "https://groupbenefits.manulife.com";
...
export const manulifeExecuteRequestSchema = z.object({
...
export type ManulifeExecuteRequest = z.infer<typeof manulifeExecuteRequestSchema>;
...
async function runPortalAutomation(
  ...
  try {
    ...
    // ── 1. Navigate to login ──────────────────────────────────────────────────
    // TODO: Verify login URL against Manulife Group Benefits portal
    ...
    // ── 2. Fill login form ────────────────────────────────────────────────────
    // TODO: Verify username input selector against Manulife portal
    await page.waitForSelector(
      "#username, input[name='username'], input[type='email'], #loginId",
      { timeout: stepTimeoutMs }
    );
    await page.fill(
      "#username, input[name='username'], input[type='email'], #loginId",
    ...
    // TODO: Verify password input selector against Manulife portal
    await page.fill(
      "#password, input[name='password'], input[type='password']",
      password
    );

    // TODO: Verify submit button selector against Manulife portal
    await page.click(
      "button[type='submit'], input[type='submit'], .btn-primary, #loginButton"
    );
    ...
    await Promise.race([
      page
        .waitForSelector(
          ".error-message, .alert-danger, [data-testid='login-error'], .notification--error",
          { timeout: 4_000 }
        )
        .then(() => "error"),
      page
        .waitForURL(/dashboard|home|benefits|provider|plan/, {
          timeout: stepTimeoutMs
        })
        .then(() => "success")
    ]).catch(() => undefined);

    // TODO: Verify login error selector against Manulife portal
    ...
    // TODO: Verify claims navigation selector against Manulife portal
    await page.waitForSelector(
    ...
    // TODO: Verify claim number input selector against Manulife portal
    await page.waitForSelector(
    ...
    // TODO: Verify search submit button selector against Manulife portal
    await page.click(
      ".claim-search button[type='submit'], .search-claims-btn, button.search, #searchButton"
    );

    // ── 6. Wait for results ───────────────────────────────────────────────────
    // TODO: Verify results container selector against Manulife portal
    ...
    if (!resultsEl) {
      // TODO: Verify not-found message selector against Manulife portal
      const notFoundEl = await page.$(
        ".no-results, .claim-not-found, [data-testid='no-results'], .no-claims"
      );
      if (notFoundEl) {
        throw new PortalError(
          "data_missing",
          `Claim ${claimNumber} not found in Manulife Group Benefits portal`
        );
      }
      throw new PortalError(
        "portal_changed",
        "Could not find results container — Manulife portal layout may have changed"
      );
    }

    // ── 7. Extract claim status ───────────────────────────────────────────────
    // TODO: Verify status cell selector against Manulife portal
    ...
    if (!statusEl) {
      throw new PortalError(
        "portal_changed",
        "Could not find claim status element — Manulife portal layout may have changed"
      );
    }
    ...
    if (!statusText?.trim()) {
      throw new PortalError(
        "portal_changed",
        "Claim status element was empty — Manulife portal layout may have changed"
      );
    }

    // ── 8. Extract paid amount ────────────────────────────────────────────────
    // TODO: Verify paid amount selector against Manulife portal
    ...
    // TODO: Verify denial reason selector against Manulife portal
...
function buildSummary(payload: ManulifePayload): string {
  const base = `Manulife Group Benefits: claim ${payload.claimNumber ?? "unknown"}`;
...
export async function executeManulife(
  request: ManulifeExecuteRequest
  ...
  if (!row) {
    return buildFailureObservation(
      "authentication",
      false,
      `No Manulife Group Benefits credentials configured for org ${request.orgId}`,
      Date.now() - startMs
    );
  }
  ...
  try {
    ...
    return buildFailureObservation(
      "authentication",
      false,
      "Failed to decrypt Manulife credentials",
      Date.now() - startMs
    );
  }
  ...
  try {
    ...
    const parsed = manulifePayloadSchema.safeParse({
      connectorId: CONNECTOR_ID,
    ...
    const payload: ManulifePayload = parsed.success
      ? parsed.data
      : {
          connectorId: CONNECTOR_ID,
          claimNumber: request.claimContext.claimNumber,
    ...
    const summary = isTimeout
      ? "Manulife Group Benefits portal timed out"
      : `Manulife automation failed: ${err instanceof Error ? err.message : "unknown error"}`;
